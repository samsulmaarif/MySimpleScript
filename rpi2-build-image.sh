...
########################################################################
# Modifikasi oleh Samsul Ma'arif <info@samsul.web.id>, 
# 22 Januari 2016
# 
# Membuat tiOS versi rasberry pi
# Skrip asli diambil dari : http://www.finnie.org/software/raspberrypi/rpi2-build-image.sh
########################################################################
...
# Standard packages
chroot $R apt-get -y install ubuntu-standard xubuntu-desktop initramfs-tools raspberrypi-bootloader-nokernel rpi2-ubuntu-errata language-pack-en
...
# Set up default user
chroot $R adduser --gecos "pengguna tiOS" --add_extra_groups --disabled-password tios
chroot $R usermod -a -G sudo,adm -p '$6$KlHMVi6Z$22Lusi/HvJCc87ozY1iOBLmn92py2gOxVTAr9.0JXO/0SjW3yuVVP7VbwrZLFwla4exKZnxcotSDDaa./uszA/' tios
...
DATE="$(date +%Y-%m-%d)"
dd if=/dev/zero of="$BASEDIR/${DATE}-tios-${RELEASE}.img" bs=1M count=1
dd if=/dev/zero of="$BASEDIR/${DATE}-tios-${RELEASE}.img" bs=1M count=0 seek=1792
sfdisk -f "$BASEDIR/${DATE}-tios-${RELEASE}.img" <<EOM
unit: sectors
...
EOM
VFAT_LOOP="$(losetup -o 1M --sizelimit 64M -f --show $BASEDIR/${DATE}-tios-${RELEASE}.img)"
EXT4_LOOP="$(losetup -o 65M --sizelimit 1727M -f --show $BASEDIR/${DATE}-tios-${RELEASE}.img)"
mkfs.vfat "$VFAT_LOOP"
mkfs.ext4 "$EXT4_LOOP"
...
if which bmaptool; then
  bmaptool create -o "$BASEDIR/${DATE}-tios-${RELEASE}.bmap" "$BASEDIR/${DATE}-tios-${RELEASE}.img"
fi
